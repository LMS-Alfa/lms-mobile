...
import { SubjectGrade as ExportedSubjectGrade, GradeItem as ExportedGradeItem } from './gradesService';
...
interface StudentClass {
  ...
  } | null[] | null; // Can be object, array of objects, or null
}
...
interface LessonFromSupabase {
  id: any;
  lessonname: any;
  lessondate?: any; // Make optional if not always present
...
export const fetchParentChildren = async (parentId: string): Promise<ChildData[]> => {
  try {
    ...
    // Format the data
    const childrenData: ChildData[] = await Promise.all(students.map(async (student: any) => {
...
export const fetchChildGrades = async (childId: string): Promise<SubjectGrade[]> => {
  try {
    ...
    console.log(`Found ${attendanceRecords?.length || 0} total attendance records for student ${childId}`);
      ...
      for (const grade of grades) {
        try {
          ...
          if (Array.isArray(grade.lesson)) {
            ...
            lessonDetails = grade.lesson as { id: any; lessonname: string; subjectid: string; } || null;
          }
      ...
      for (const attendance of attendanceRecords) {
        try {
          ...
          if (Array.isArray(attendance.lesson)) {
            ...
            lessonDetails = attendance.lesson as { id: any; lessonname: string; subjectid: string; } || null;
          }
    ...
    for (const [_, subjectGrade] of subjectMap.entries()) {
      if (subjectGrade.grades.length > 0) {
        ...
        if (scores.length > 0) {
          subjectGrade.numericGrade = scores.reduce((a: number, b: number) => a + b, 0) / scores.length;
...
const mapAnnouncementToNotificationType = (targetAudience: string): 'grade' | 'attendance' | 'behavior' | 'announcement' | 'event' => {
...
export const fetchStudentGrades = async (studentId: string) => {
  try {
    ...
    // Process and format the data
    const gradesResult = data?.map(grade => {
      const lessonData = Array.isArray(grade.lesson) ? grade.lesson[0] : grade.lesson as LessonFromSupabase | null;
      ...
      if (Array.isArray(subjectData)) {
        ...
        finalSubjectName = (subjectData as { subjectname: string }).subjectname || 'Unknown Subject';
      }

      const quarterData = Array.isArray(grade.quarter) ? grade.quarter[0] : grade.quarter as QuarterFromSupabase | null;
      ...
      return {
        ...
        subjectName: finalSubjectName, // Use the correctly derived subject name
        subjectColor:  '#4A90E2',
...
export const fetchStudentAssignments = async (studentId: string) => {
  try {
    ...
    // Process and format the data
    const assignmentsResult = data?.map(assignment => {
...
export const fetchStudentSubmissions = async (studentId: string) => {
  try {
    ...
    // Process and format the data
    const submissionsResult = data?.map(submission => {
      ...
      return {
        ...
        isLate: submission.submittedat && assignmentData?.duedate ?
          new Date(submission.submittedat) > new Date(assignmentData.duedate) : false,
        subjectName: subjectNameStr,
        subjectColor: '#4A90E2',
...
export const fetchStudentAttendance = async (studentId: string) => {
  try {
    ...
    // Process and format the data
    const attendanceResult = data?.map(record => {
      const lessonData = Array.isArray(record.lesson) ? record.lesson[0] : record.lesson as LessonFromSupabase | null;
      const subjectData = lessonData?.subject;
      let finalSubjectName = 'Unknown Subject';
      if (Array.isArray(subjectData)) {
        finalSubjectName = subjectData[0]?.subjectname || 'Unknown Subject';
    ...
    const statistics = {
      ...
      presentPercentage: attendanceResult.length > 0 ? 
        (attendanceResult.filter(r => r.status === 'present').length / attendanceResult.length) * 100 : 0
    };
    ...
    return {
      profile: {
        ...
        parent: studentParentData ? {
          ...
          name: `${studentParentData.firstName || ''} ${studentParentData.lastName || ''}`.trim(),
  ...
  try {
    ...
    const attendanceRecords = data?.map(record => {
      ...
      let lessonData: { 
        ...
        lessondate: string; 
        subjectid: string; 
        subject: { id: string; subjectname: string; }[] | { id: string; subjectname: string; } | null; 
      } | null = null;
      ...
      if (lessonData?.subject) {
        ...
        } else if (lessonData.subject) { // Check if it's not null
...
export const fetchParentChildAssignments = async (childId: string, parentId: string): Promise<ChildAssignment[]> => {
  try {
...
export const fetchParentDashboard = async (parentId: string) => {
  try {
    ...
    const avgGpa = validChildren.length > 0 
      ? validChildren.reduce((sum: number, child) => sum + parseFloat(child.performanceSummary.gpa), 0) / validChildren.length
      : 0;
      
    const upcomingTestsCount = validChildren.reduce((sum: number, child) => 
      sum + child.performanceSummary.upcomingTests, 0);
      
    const recentAbsences = validChildren.reduce((sum: number, child) => 
      sum + child.recentAttendance.absent, 0);
...
export const fetchChildAssignmentDetails = async (childId: string, assignmentId: string): Promise<ParentDetailedAssignment | null> => {
  try {
    ...
    if (submissionError) {
      console.error(`Error fetching submission for assignment ${assignmentId}, child ${childId}:`, submissionError);
      ...
      if (submissionError.code !== 'PGRST116') { // PGRST116: Row not found (expected if no submission)
    ...
    if (isCompleted) {
      statusText = submission?.grade !== null && submission?.grade !== undefined ? 'Graded' : 'Submitted';
    } else if (isPastDue) {
      statusText = 'Overdue';
    } else {
    ...
    let subjectNameStr = 'Unknown Subject';
    if (assignment.subject && Array.isArray(assignment.subject) && assignment.subject.length > 0 && assignment.subject[0]) {
        ...
        subjectNameStr = (assignment.subject as { subjectname: string }).subjectname || 'Unknown Subject';
    }
...
export const fetchChildTimetable = async (childId: string): Promise<TimetableEntry[]> => {
  try {
    ...
    const uniqueSubjectIds = Array.from(new Set(timetableEntriesData.map(entry => entry.subjectId).filter(id => id !== null))) as string[];
    ...
    if (uniqueSubjectIds.length > 0) {
        ...
        if (ctError) {
            ...
            classTeacherData.forEach((assignment: any) => {
                ...
                if (assignment.teacher) { // teacher should be an object due to !teacherid syntax
    ...
    return timetableEntriesData.map((entry: any) => {
      ...
      if (assignedTeacher) {
          teacherName = `${assignedTeacher.firstName || ''} ${assignedTeacher.lastName || ''}`.trim() || 'N/A';
      }
...
export const fetchParentChildrenList = async (parentId: string): Promise<ParentChildListItem[]> => {
  try {
    ...
    if (error) {
      console.error(`[parentSupabaseService] Error fetching children for parent ${parentId}:`, error);
    ...
    console.log(`[parentSupabaseService] Successfully fetched ${childrenList.length} children for parent ${parentId}.`);
    return childrenList;

  } catch (e: any) {
    console.error(`[parentSupabaseService] Exception in fetchParentChildrenList for parent ${parentId}:`, e);
